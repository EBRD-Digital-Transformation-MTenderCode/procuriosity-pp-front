<template>
  <el-collapse-item :name="gd(fs, _ => _.compiledRelease.ocid)">
    <template slot="title">
      <div class="info-blocks accordion-header ">
        <div class="info-block accordion-table">
          <el-row :gutter="15">
            ...
            <el-col :sm="3">
              <div
                :data-th="$t('budget.source_of_money')"
            ...
            <el-col :sm="4">
              <div
                :data-th="$t('budget.validity_period')"
                class="info-block__text info-block__text_bold accordion-table__th"
              >
            ...
            <el-col :sm="4">
              <div :data-th="$t('budget.amount')" class="info-block__text info-block__text_bold accordion-table__th">
                {{ fa(gd(fs, _ => _.compiledRelease.planning.budget.amount.amount)) }}
                {{ gd(fs, _ => _.compiledRelease.planning.budget.amount.currency) }}
              </div>
            </el-col>
            <el-col :sm="3">
              <div :data-th="$t('budget.status')" class="info-block__text info-block__text_bold accordion-table__th">
                {{
                  gd(fs, _ => _.compiledRelease.planning.budget.status)
                    ? $t("budget.verified")
                    : $t("budget.not_verified")
                }}
              </div>
            </el-col>
          </el-row>
      ...
      <div class="info-blocks">
        <el-row :gutter="15">
          ...
          <el-col :sm="16">
            <div class="info-block__text">
              {{
                gd(fs, _ => _.compiledRelease.planning.budget.isEuropeanUnionFunded)
                  ? $t("budget.yes")
                  : $t("budget.no")
              }}
            </div>
...
<script>
import { getDataFromObject, formatDate, formatAmount, getOrganizationName } from "./../../../../utils";
