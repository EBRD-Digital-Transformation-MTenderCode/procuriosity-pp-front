<template>
  <el-collapse-item :name="gd(ms, _ => _.ocid)">
    <template slot="title">
      <div class="info-blocks accordion-header">
        <div class="info-block accordion-table">
          <el-row :gutter="15">
            ...
            <el-col :sm="5">
              <div
                :data-th="$t('budget.current_status')"
                class="info-block__text info-block__text_bold accordion-table__th"
              >
                {{ mapProcedureStatus }}
              </div>
            </el-col>
            <el-col :sm="5">
              <div
                :data-th="$t('budget.expected_value')"
                class="info-block__text info-block__text_bold accordion-table__th"
              >
      ...
      <div class="info-blocks">
        <el-row :gutter="15">
          ...
          <el-col :sm="16">
            <div class="info-block__text">
              <a
                :href="`${$i18n.locale !== 'ro' ? `/${$i18n.locale}` : ''}/${getEntityName}/${gd(ms, _ => _.ocid)}`"
                target="_blank"
                data-link
...
<script>
import { getDataFromObject, formatAmount, getOrganizationObject, selectProcedure } from "./../../../../utils";
...
export default {
  ...
  computed: {
    ...
    activeStage() {
      return this.procedure.hasOwnProperty("EV")
        ? this.gd(this.procedure, _ => _.EV)
        : this.gd(this.procedure, _ => _.PN);
    },
    mapProcedureStatus() {
      if (this.activeStage.ocid.match(/^ocds-([a-z]|[0-9]){6}-[A-Z]{2,}-[0-9]{13}-PN-[0-9]{13}$/)) {
        ...
        const status = `${this.activeStage.compiledRelease.tender.status}.${
          this.activeStage.compiledRelease.tender.statusDetails
        }`;
        switch (status) {
    ...
    getEntityName() {
      return this.activeStage.ocid.match(/^ocds-([a-z]|[0-9]){6}-[A-Z]{2,}-[0-9]{13}-PN-[0-9]{13}$/)
        ? "plans"
        : "tenders";
    },
  },
